#!/usr/bin/env python3
# NegotiationBERT.py
# Fine-tuning pretrained DeBERTa-v3 for negotiation code classification
#
# Uses HuggingFace transformers library with pretrained DeBERTa-v3 model
# DeBERTa-v3 is more advanced than BERT with significant improvements:
# - Disentangled attention mechanism (separate content & position embeddings)
# - Enhanced mask decoder for better understanding
# - Superior performance on GLUE/SuperGLUE benchmarks
# Benefits:
# - State-of-the-art bidirectional context understanding
# - Better classification accuracy than BERT/RoBERTa
...
parser = argparse.ArgumentParser(description='Train NegotiationBERT (DeBERTa-v3) model')
...
# Model config - Using pretrained DeBERTa-v3
MODEL_NAME = "microsoft/deberta-v3-base"  # Options: "microsoft/deberta-v3-small", "microsoft/deberta-v3-base", "microsoft/deberta-v3-large"
MAX_SEQ_LEN = 512  # DeBERTa-v3 supports longer sequences efficiently
DROPOUT_RATE = 0.15  # Lower dropout since DeBERTa is very robust

# Training config
EPOCHS = args.epochs
BATCH_SIZE = 16  # Can use larger batch size for classification (no generation task)
...
# Loss weights
LAMBDA_CODE = 1.0
LAMBDA_SPEAKER = 0.5  # Optional speaker classification
LAMBDA_CORR = 0.1  # Optional correctness classification
...
# Composite score weights (for BERT, we focus on classification accuracy)
...
class NegotiationBERT(nn.Module):
    def __init__(self, model_name=MODEL_NAME):
        ...
        print(f"[Model] Loading DeBERTa-v3 model: {model_name}")
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None):
        ...
        # DeBERTa uses first token ([CLS]) as sequence-level representation
        # DeBERTa doesn't have pooler_output, so we take the first token manually
    ...
    def __init__(
        ...
        for transcript_name, conv_items in conversations.items():
            for idx, item in enumerate(conv_items):
                ...
                # Tokenize with HuggingFace BERT tokenizer
                encoding = tokenizer(
                    full_input,
    ...
    def evaluate_sliding_window(self, dataset, window_size=SLIDING_WINDOW_SIZE):
        ...
        with torch.no_grad():
            for start_idx in range(0, len(dataset) - window_size + 1):
                ...
                token_type_ids = target_sample['token_type_ids'].unsqueeze(0).to(DEVICE)
                code_label = target_sample['code_label'].to(DEVICE)

                code_logits, _, _ = self.model(input_ids, attention_mask, token_type_ids)
...
if __name__ == "__main__":
    ...
    model = NegotiationBERT(MODEL_NAME)

    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())

    print(f"\n--- NegotiationDeBERTa (Pretrained DeBERTa-v3-base) ---")
    ...
    for epoch in range(EPOCHS):
        ...
        # Compute composite score (for BERT, focus on code accuracy)
