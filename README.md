...
**Created by:** Alara Kaymak and Ryan Li
...
NegotiationGPT now includes a fully-featured web interface for real-time negotiation assistance:

**Features:**
- **Interactive Chat**: Real-time conversation with AI negotiation assistant
- **Model Selection**: Choose between T5, BERT, RoBERTa, ALBERT, DistilBERT, and DistilGPT2 models
- **Role Selection**: Switch between Buyer, Seller, or User perspectives
- **Smart Suggestions**: Get AI-powered negotiation recommendations with "Use This" button
- **Conversation History**: Save and manage multiple chat sessions
- **Real-time Predictions**: View negotiation codes and speaker predictions for each message
- **Alternating Message Display**: User messages on right (purple), AI responses on left (white)

**Quick Start:**
```bash
# Option 1: Launch with specific model
python start_chat_with_model.py
...
**Architecture:**
- **Frontend**: `static/index.html` with vanilla JavaScript (`static/js/app.js`)
- **Backend**: Flask server (`chat_server.py`) with model inference (`chat_inference.py`)
- **Storage**: Chat sessions saved in `chat_sessions.json`
...
Training results are stored in each model's directory (e.g., `RoBERTa/results/`, `BERT/results/`, etc.) with accuracy and loss plots.
...
Training and validation plots are automatically generated during training and saved in each model's `results/` directory:
- `loss_plot.png` - Training and validation loss curves
- `accuracy_plot.png` - Training and validation accuracy curves
- `test_accuracy_plot.png` - Final test accuracy
...
### Systematic Architecture Exploration: Multiple Pretrained Models

After establishing that training from scratch was fundamentally limited by data scale, we systematically explored multiple pretrained architectures to identify the optimal model for our negotiation classification task. The current implementation includes six pretrained models, each offering unique advantages:

| Model | Parameters | Key Advantage | Use Case |
|-------|------------|---------------|----------|
| **RoBERTa** | 125M (1.9M trainable) | Best overall accuracy | Production deployment |
| **BERT** | 110M (1.7M trainable) | Strong baseline, well-documented | Research comparison |
| **T5** | 60M (encoder-decoder) | Generative capabilities | Text generation tasks |
| **ALBERT** | 18M (parameter sharing) | Memory efficient | Resource-constrained deployment |
| **DistilBERT** | 66M (distilled) | Fast inference, 60% speedup | Real-time applications |
| **DistilGPT2** | 82M (generative) | Decoder-only architecture | Alternative approach |

All models are accessible through the web interface with easy model switching.
...
Training outputs are saved to `DistilBERT/results/` including loss plots, accuracy plots, and model weights.
...
Training outputs are saved to `ALBERT/results/` including loss plots, accuracy plots, and model weights.
...
#### **Option 1: Train Pretrained Models (Recommended)**

Train any of the pretrained transformer models:

```bash
# RoBERTa (Best Performance)
python3 NegotiationRoBERTa.py
# Output: RoBERTa/final_model.pt, RoBERTa/results/

# BERT
python3 NegotiationBERT.py
# Output: BERT/final_model.pt, BERT/results/

# T5 (Encoder-Decoder)
python3 NegotiationT5.py
# Output: T5/final/pytorch_model.bin, T5/results/

# ALBERT (Parameter-Efficient)
python3 NegotiationALBERT.py
# Output: ALBERT/final_model.pt, ALBERT/results/

# DistilBERT (Faster, Smaller)
python3 NegotiationDistilBERT.py
# Output: DistilBERT/final_model.pt, DistilBERT/results/

# DistilGPT2 (Generative)
python3 NegotiationDistilGPT2.py
# Output: DistilBERTGPT2/final_model.pt, DistilBERTGPT2/results/
```

Each model trains for multiple epochs with early stopping and generates:
- `final_model.pt` - Trained model weights
- `final_tokenizer/` - Tokenizer configuration
- `results/` - Training plots and metrics

#### **Option 2: Train Custom GPT (From Scratch)**

```bash
# Train custom transformer from scratch
python NegotiationGPT.py

# Note: Requires significantly more data for good performance
# Expected test accuracy: ~44% on small dataset
```

#### **Option 3: Quick Model Evaluation**

```bash
# Use the model selection example
python model_selection_example.py

# Or test individual predictions with the web interface
python start_chat_with_model.py
```
...
python start_chat_with_model.py
# Then open http://localhost:5000
# Select model from dropdown: T5, BERT, RoBERTa, ALBERT, DistilBERT, or DistilGPT2
```
...
# Load RoBERTa model (best performance)
model_path = "RoBERTa/final_model.pt"
tokenizer = AutoTokenizer.from_pretrained("RoBERTa/final_tokenizer")

# Load model architecture
base_model = AutoModelForSequenceClassification.from_pretrained(
    "roberta-base",
    num_labels=27  # 27 negotiation codes
)
...
print(f"Predicted negotiation code: {prediction.item()}")
```

**Option 3: Model Selection Example**

```python
# See model_selection_example.py for complete example
python model_selection_example.py
```

**Available Models:**
- **RoBERTa** (`RoBERTa/final_model.pt`) - Best overall performance
- **BERT** (`BERT/final_model.pt`) - Strong baseline
- **T5** (`T5/final/`) - Encoder-decoder architecture
- **ALBERT** (`ALBERT/final_model.pt`) - Parameter-efficient
- **DistilBERT** (`DistilBERT/final_model.pt`) - Fast inference
- **DistilGPT2** (`DistilBERTGPT2/final_model.pt`) - Generative approach
...
│   ├── NegotiationRoBERTa.py              # RoBERTa-based model
│   ├── NegotiationBERT.py                 # BERT-based model
│   ├── NegotiationT5.py                   # T5-based model
│   ├── NegotiationALBERT.py               # ALBERT-based model
│   ├── NegotiationDistilBERT.py           # DistilBERT-based model
│   ├── NegotiationDistilGPT2.py           # DistilGPT2-based model
│   ├── NegotiationMistral.py              # Mistral-based model
│   ├── NegotiationGPT.py                  # Custom transformer (GPT-style)
│   └── NegotiationGPT_Pretrained.py       # Pretrained variant
│
├── Model Directories/ (Training Outputs)
│   ├── RoBERTa/                           # RoBERTa training results
│   │   ├── final_model.pt                 # Trained model weights
│   │   ├── final_tokenizer/               # Tokenizer files
│   │   └── results/                       # Plots and metrics
│   ├── BERT/                              # BERT training results
│   ├── T5/                                # T5 training results
│   ├── ALBERT/                            # ALBERT training results
│   ├── DistilBERT/                        # DistilBERT training results
│   └── DistilBERTGPT2/                    # DistilGPT2 training results
│
├── Web Interface/
│   ├── static/
...
│   ├── chat_inference.py                  # Model inference logic
│   └── chat_sessions.json                 # Chat session storage
│
├── Training & Utilities/
│   ├── start_chat_with_model.py           # Launch chat with model selection
│   ├── model_selection_example.py         # Model selection examples
│   ├── negotiation_analytics.py           # Analytics utilities
│   ├── rlhf_trainer.py                    # RLHF training utilities
│   └── start_chat.sh / .bat               # Quick start scripts
│
├── Data/
│   ├── Sonnet4-consolidated.csv           # Main training dataset (~1,300 samples)
│   ├── negotiation_dataset_5000.csv       # Extended dataset
...
│   ├── CHAT_INTERFACE_GUIDE.md            # Web interface guide
│   ├── PRETRAINED_MODELS_GUIDE.md         # Model selection guide
│   ├── START_GUIDE.md                     # Setup instructions
│   ├── UPDATE_SUMMARY.md                  # Recent updates
│   ├── RESEARCH_ENHANCEMENTS.md           # Research notes
│   ├── MODEL_WEIGHTS_FIX.md               # Troubleshooting
│   └── FAILSAFE_IMPLEMENTATION.md         # Error handling
│
├── Architectural Diagrams/
│   └── architecture_diagrams/
│       ├── NegotiationGPT_architecture.svg
│       └── NegotiationGPT_Pretrained_architecture.svg
│
...
**Web Interface:**
- Full-featured chat interface for real-time negotiation assistance
- Multi-model support with easy switching (T5, BERT, RoBERTa, ALBERT, DistilBERT, DistilGPT2)
- Smart suggestion system with "Use This" button
- Conversation history and session management
- Alternating message display (user/AI differentiation)

**Model Implementations:**
- Six pretrained transformer models fully implemented and trained
- Organized model directories with training outputs and plots
- Automatic visualization generation (loss curves, accuracy plots)
- Standardized model loading through web interface

**Architecture:**
- Modular design with separate inference and server components
...
Training results are stored in each model's directory:
- **RoBERTa/results/** - Accuracy and loss plots from RoBERTa training
- **BERT/results/** - BERT training visualizations
- **T5/results/** - T5 training metrics
- **ALBERT/results/** - ALBERT training outputs
- **DistilBERT/results/** - DistilBERT performance plots
- **DistilBERTGPT2/results/** - DistilGPT2 training curves

Each results directory contains:
- `loss_plot.png` - Training and validation loss progression
- `accuracy_plot.png` - Training and validation accuracy curves
- `test_accuracy_plot.png` - Final test performance
- `vocab.json` - Model vocabulary for inference

**Note:** Specific numerical results depend on the training run. To view results:
1. Train a model using instructions in Section 7
2. Check the model's `results/` directory for plots and metrics
3. View terminal output during training for epoch-by-epoch performance
...
- **Created by:** Alara Kaymak and Ryan Li
- **Models:** RoBERTa-base (Meta AI), BERT (Google), T5 (Google), ALBERT (Google), DistilBERT (Hugging Face), LoRA (Hugging Face PEFT)
- **Libraries:** PyTorch, Transformers, PEFT, Flask, Optuna
