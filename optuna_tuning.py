...
# 
# UPDATED: Added early stopping and stronger regularization to prevent overfitting
# - Early stopping: Stops if no improvement for 5 epochs
# - Stronger regularization: Dropout 0.3-0.5, Weight decay 0.1-0.3
# - Reduced model size: d_model 256-512 (was 256-768)
# - More trials: 50 (was 20) for better hyperparameter search
...
# Optuna study config
N_TRIALS = 50  # Number of hyperparameter trials to run (increased for better search)
...
def objective(trial: optuna.Trial) -> float:
    ...
    # Suggest hyperparameters (UPDATED: Stronger regularization to prevent overfitting)
    params = {
        # Model architecture (reduced size to prevent overfitting)
        'd_model': trial.suggest_int('d_model', 256, 512, step=128),  # Reduced from 768
        'num_layers': trial.suggest_int('num_layers', 4, 6, step=1),  # Reduced from 8
        'num_heads': trial.suggest_categorical('num_heads', [4, 8]),  # Reduced from [4,8,16]
        'dropout_rate': trial.suggest_float('dropout_rate', 0.3, 0.5, step=0.05),  # Increased from 0.05-0.3
        
        # LoRA parameters
        'lora_rank': trial.suggest_int('lora_rank', 4, 12, step=2),  # Reduced from 16
        'lora_alpha': trial.suggest_int('lora_alpha', 8, 24, step=4),  # Reduced from 32
        
        # Training parameters
        'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True),
        'batch_size': trial.suggest_categorical('batch_size', [4, 8, 16]),
        'weight_decay': trial.suggest_float('weight_decay', 0.1, 0.3, step=0.05),  # Increased from 0.001-0.1
...
def train_with_best_params(best_params: Dict[str, Any]):
    ...
    # Train for full epochs with early stopping
    ...
    for epoch in range(FULL_EPOCHS):
        ...
        for i, (trainer, label_col) in enumerate(zip(trainers, VOTER_LABEL_COLS)):
            ...
            train_acc = train_metrics['code_acc']
            val_acc = val_metrics['code_acc']
            epoch_avg_val_acc += val_acc
            
            # Check for overfitting
            if train_acc > 0.95 and val_acc < 0.5:
                print(f"  ⚠️  [{label_col}] Overfitting: Train {train_acc:.3f} >> Val {val_acc:.3f}")
        ...
        if epochs_without_improvement >= EARLY_STOPPING_PATIENCE:
            ...
            print(f"Best average validation accuracy: {best_overall_val_acc:.4f}")
            print(f"{'='*60}")
            break
